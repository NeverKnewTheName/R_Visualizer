...
HugeQVector<Msg> CsvMsgPacketHandler::parseCsvMsgPacket(QString &csvMsgPacketString)
{
    ...
    msgs.append(new Msg(QDateTime(), static_cast<quint16>(0xFFu), static_cast<quint16>(code.toInt(0,16)), dataBytes));
    ...
    for(auto &msg : msgsFromPacket )
    {
        ...
        msgs.append(new Msg(QDateTime(), static_cast<quint16>(0xFFu), static_cast<quint16>(0x0B), dataBytes));
    }
    return msgs;
}
