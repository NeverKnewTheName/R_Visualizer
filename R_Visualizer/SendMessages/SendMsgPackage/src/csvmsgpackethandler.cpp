...
QVector<Msg> CsvMsgPacketHandler::parseCsvMsgPacket(QString &csvMsgPacketString)
{
    ...
    msgs.append(Msg(MsgIDType(0xFFu), MsgCodeType(code.toInt(0,16)), dataBytes));
    ...
    for(auto &msg : msgsFromPacket )
    {
        ...
        msgs.append(Msg(MsgIDType(0xFFu), MsgCodeType(0x0B), dataBytes));
    }

    MsgDataType msgDataFirstMsg = msgs.at(0).getMsgData(); //setDataByte(1,rowCntr);
    msgDataFirstMsg.replace(1,MsgDataByteType(rowCntr));
    msgs[0].setMsgData(msgDataFirstMsg);
